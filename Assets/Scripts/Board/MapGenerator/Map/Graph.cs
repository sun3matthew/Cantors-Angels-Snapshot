...
namespace Assets.Map
{
    public class Graph
    {
        List<KeyValuePair<int, Corner>> _cornerMap = new();
        ...
        private CoreRandom _coreRandom;

        public Graph(IEnumerable<Vector2> points, Voronoi voronoi, int width, int height, float lakeThreshold, CoreRandom coreRandom)
        {
            _coreRandom = coreRandom;
            Width = width;
            Height = height;
            inside = IslandShape.makePerlin(coreRandom.Next(100000));
        ...
        private void BuildGraph(IEnumerable<Vector2> points, Delaunay.Voronoi voronoi)
        {
            ...
            foreach (var libedge in libedges)
            {
                ...
                if (edge.d0 != null && vedge.p0 != null) { edge.d0.borders.Add(edge); }
                if (edge.d1 != null && vedge.p1 != null) { edge.d1.borders.Add(edge); }
                ...
                foreach (var s in q.adjacent)
                {
                    ...
                    if (!q.water && !s.water)
                    {
                        ...
                        if (_needsMoreRandomness)
                        {
                            ...
                            newElevation += _coreRandom.NextFloat();
                        }
                    }
        ...
        private void CreateRivers()
        {
            ...
            for (var i = 0; i < (Width + Height) / 4; i++)
            {
                ...
                var q = corners[_coreRandom.Next(corners.Count - 1)];
        ...
        static Biome GetBiome(Center p)
        {
            ...
            {
                // if (p.elevation < 0.2) return Biome.Marsh;
            ...
            {
                if (p.moisture > 0.83) return Biome.PineForest;
                else if (p.moisture > 0.50) return Biome.GinkgoForest;
                else if (p.moisture > 0.16) return Biome.Grassland;
                else return Biome.TemperateDesert;
            }
            else
            {
                if (p.moisture > 0.66) return Biome.RainForest;
                else if (p.moisture > 0.33) return Biome.OakForest;
                else if (p.moisture > 0.16) return Biome.Grassland;
